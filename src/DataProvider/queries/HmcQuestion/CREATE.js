...
export const createHmcQuestionMutation = gql`
  mutation CreateHmcQuestion($input: CreateHmcQuestionInput!) {
    data: createHmcQuestion(input: $input) {
      id
      orderIndex
      localisations {
        language
...
export default async ({ client, params }) => {
  try {
    ...
    const result = await client.mutate({
      mutation: createHmcQuestionMutation,
      variables: {
